...
The **Domain Name System** is a system (client‑server architecture) used to translate human‑friendly domain names (e.g., example.com) into IP addresses (e.g., 93.184.216.34) and vice‑versa. 
It lives at the application layer of the TCP/IP stack and uses UDP on port 53 to communicate. 

It is built on a hierarchical distributed database, where many name servers (NS) cooperate.
...
First checks the hosts file (a static mapping of domain → IP on your PC).

Then checks its own cache of previously‐resolved records.

If the record is not found locally, then it queries the Local NS
