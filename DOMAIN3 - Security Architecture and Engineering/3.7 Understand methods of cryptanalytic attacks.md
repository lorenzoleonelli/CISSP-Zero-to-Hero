...
> ℹ Known plaintext attacks are based on **cribs**. In cryptography, a crib is a piece of plaintext that the attacker expects or guesses to appear in the encrypted message (ciphertext).
...
flowchart TB
  ...
  Success -->|No| Iterate["Refine hypotheses or collect more pairs"]
  Iterate --> Analyse
...
> ℹ Frequency analysis is particularly effective against simple ciphers, like the Caesar cipher or monoalphabetic substitution cipher, because these ciphers have predictable patterns that can be exploited. 
...
These attacks are particularly dangerous because even if a cryptographic algorithm is strong, side-channel leaks can still provide attackers with the means to extract sensitive data. 

To defend against side-channel attacks, it is essential to implement countermeasures like constant-time algorithms, shielding against physical leaks, and ensuring that systems are resistant to timing and power analysis.
...
> ⚠ These attacks are particularly effective when the system does not take precautions to ensure that its operations take constant time, regardless of the input. Insecure implementations that process data in variable times based on its content are more vulnerable to timing attacks.
...
> ⚠ MITM attacks can be particularly dangerous in cryptographic systems that rely on key exchange mechanisms, such as public key cryptography. If an attacker successfully intercepts and manipulates the keys during the key exchange process, they can decrypt all subsequent communications, potentially compromising sensitive data like passwords, financial transactions, or personal information.
...
<details>
  <summary>Show answer</summary>
A PTT attack occurs when an attacker steals a valid Kerberos ticket, such as a Ticket-Granting Ticket (TGT), from a compromised system. With the stolen ticket, the attacker can impersonate the legitimate user and request access to other network services. This is dangerous because it enables lateral movement across the network without needing the user’s actual password.
</details>

2. How can attackers forge Kerberos tickets, and what advantage does this give them?
<details>
  <summary>Show answer</summary>
Attackers can forge tickets (such as creating a "Golden Ticket") by exploiting weak cryptographic keys, compromised service accounts, or misconfigurations. A forged ticket allows the attacker to impersonate any user, even domain administrators, giving them virtually unlimited access to the network.
</details>

3. What security measures help defend against Kerberos exploitation?
<details>
  <summary>Show answer</summary>
Defenses include regularly rotating service account passwords, enforcing mutual authentication, enabling multi-factor authentication (MFA), properly configuring Kerberos realms, and monitoring for unusual ticket activity. Protecting systems that store Kerberos keys (like domain controllers) is also critical, since their compromise enables powerful ticket forging attacks.
</details>
...
<details>
  <summary>Show answer</summary>
Ransomware employs strong encryption algorithms (such as AES or RSA) to encrypt files on the victim’s system. Without the attacker’s private decryption key, the victim cannot access their data, making the ransom payment the only quick option to recover it.
</details>

2. Why is paying the ransom considered risky and not always effective?
<details>
  <summary>Show answer</summary>
Paying the ransom does not guarantee that attackers will provide the decryption key or that the data will be restored intact. In some cases, attackers may demand additional payments or simply disappear, leaving the victim with encrypted, unusable files. It also funds further criminal activity.
</details>

3. What are the most effective defenses against ransomware attacks?
<details>
  <summary>Show answer</summary>
The best defenses include maintaining regular, offline backups of critical data, keeping systems and applications updated to patch vulnerabilities, and using layered security (firewalls, endpoint protection, intrusion detection). Training employees to recognize phishing attempts is equally critical since phishing is a common ransomware entry point.
</details>
