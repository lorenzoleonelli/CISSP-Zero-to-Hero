...
> 🔗 [James Anderson’s Computer Security Technology Planning Study (1972)](https://seclab.cs.ucdavis.edu/projects/history/papers/ande72.pdf) introduced three fundamental principles for security architecture: 
> 1. security functions must not be bypassed, 
> 2. they should be small enough to be easily tested for defects, 
> 3. and they must always be invoked when needed.

> ℹ *ISO/IEC 19249* provides a standardized approach to secure system design. ISO/IEC 19249 outlines **five architectural principles**:
...
```mermaid
graph LR
    %% -- Defined Subjects (Active Actors) -- %%
    subgraph SU ["SUBJECTS (Active Entities)"]
        direction LR
        S1[👨‍💼 User]
        S2[💻 Process]
    end

    %% -- Defined Objects (Passive Resources) -- %%
    subgraph OB ["OBJECTS (Passive Entities)"]
        direction LR
        O1(📂 File)
        O2(🗄️ Database)
    end

    %% -- The High-Engagement Flow -- %%
    SU ==>|"[1] Request (Active)"| OB
    OB ---|"[2] Provide Data (Passive)"| SU

    %% -- Visual Styling for Emphasis -- %%
    classDef subject fill:#444,stroke:#FFF,stroke-width:2px,color:#fff;
    classDef object fill:#AAA,stroke:#FFF,stroke-width:2px,color:#000;
...
**Transitive trust** is when trust between two systems automatically extends to a third system. 
...
graph LR
    %% -- Defined Entities -- %%
    A[System A<br/>'The Initial Truster']
    B[System B<br/>'The Middleman']
    C[System C<br/>'The Trusted Third Party']

    %% -- The Explicit Relationships -- %%
    A -- "Direct Trust" --> B
    B -- "Direct Trust" --> C

    %% -- The Transitive (Implicit) Result -- %%
    A -.->|"[Inherited Trust]"| C

    %% -- Greyscale Styling -- %%
    style A fill:#333,stroke:#000,color:#fff
    style B fill:#777,stroke:#000,color:#fff
    style C fill:#bbb,stroke:#000,color:#000
    
    %% -- Formatting the Transitive Line -- %%
...
> ℹ In PKI, transitive trust occurs when a root certificate authority (CA) trusts an intermediate CA, which then issues certificates to end entities. Any entity that trusts the root CA also implicitly trusts certificates issued by the intermediate CA.

An **open system** is designed for `interoperability`, often supporting `open standards`, public APIs, and `open-source` components. While this fosters innovation and flexibility, it also expands the attack surface, requiring strong security controls to mitigate risks. 
...
🔗 Refer also to [1.10 Understand and apply threat modeling concepts and methodologies](../DOMAIN1%20-%20Security%20and%20Risk%20Management/1.10%20Understand%20and%20apply%20threat%20modeling%20concepts%20and%20methodologies.md)
...
🔗 [The STRIDE Model in the Microsoft learning center](https://learn.microsoft.com/en-us/azure/security/develop/threat-modeling-tool-threats)
...
The principle of least privilege is a fundamental concept in cybersecurity and is crucial for minimizing potential security risks. It refers to the practice of granting users, systems, or processes only the minimum access necessary to perform their tasks or functions—no more, no less. 
...
> ✓ Layers of security ensure that if one fails, others stand strong.
...
🔗 See also 1.9.4 in [1.9 Understand and apply risk management concepts](../DOMAIN1%20-%20Security%20and%20Risk%20Management/1.9%20Understand%20and%20apply%20risk%20management%20concepts.md)
...
> ℹ  **The tyranny of the default** in information security refers to the tendency of users and organizations to stick with default settings, configurations, or behaviors, even when they are insecure. Defaults are powerful because they require effort to change, and most people assume they are safe or optimal.
...
> ⚠ Never assume that the default settings, or a standard procedure are secure.
...
2. What does the term “tyranny of the default” mean in information security?

<details> <summary>Show answer</summary> The “tyranny of the default” refers to the tendency of users and organizations to keep default settings or configurations, even when they are insecure, because changing them requires effort or expertise. </details>

3. How do password policies in modern operating systems illustrate the secure-by-default principle?

<details> <summary>Show answer</summary> Modern systems often enforce strong password requirements by default—such as length, complexity, and expiration rules—to prevent weak passwords, ensuring security even if users do not modify these settings. </details>
...
> ℹ “Fail-safe” is broader — it means the system fails in a way that minimizes harm to people, systems, or the environment. In cybersecurity, fail-safe usually leans toward fail-secure, but in safety-critical systems (like hospitals or factories), it might mean fail-open to ensure human safety.

A key principle tied to file-safe is **default deny**—which means that `unless explicitly allowed, actions should be blocked`. This is why firewalls often start with a "deny all" rule, requiring administrators to explicitly permit necessary traffic. Similarly, access to sensitive data should be granted only to authorized users instead of trying to block unauthorized ones reactively.
...
2. Why might a hospital’s access control system be designed to fail open instead of fail secure?
...
<details> <summary>Show answer</summary> The `try...catch` mechanism ensures that when an error occurs, it’s handled in a **controlled and secure way**, preventing application crashes or data exposure. This reflects the principle of designing systems that fail safely without compromising security. </details>

5. When designing secure systems, what factors should influence the choice between fail-open and fail-secure behavior?

<details> <summary>Show answer</summary> The decision depends on the system’s **purpose, risk tolerance, and priorities**. Environments prioritizing availability (like emergency services) may use fail-open, while those focused on confidentiality and integrity (like banking or cybersecurity systems) should use fail-secure. </details>
...
> ✓ Never let one hand do what two should control.
...
<details> <summary>Show answer</summary> While least privilege limits the access rights of each user to only what’s necessary for their job, **segregation of duties** focuses on dividing tasks among multiple people to prevent any one individual from having unchecked power over a process. </details>

5. In what type of organizational process is Segregation of Duties most critical?

<details> <summary>Show answer</summary> SoD is most critical in financial, administrative, and security-sensitive processes, such as approving transactions, managing user access, or deploying code changes—where a lack of oversight could result in fraud, errors, or security breaches. </details>
...
> Bruce Schneier stated: *Complexity is the worst enemy of security. The more complex you make your system, the less secure it’s going to be, because you’ll have more vulnerabilities and make more mistakes somewhere in the system. . . . The simpler we can make systems, the more secure they are.*

The following concepts are related to the KISS principle:
...
<details> <summary>Show answer</summary> YAGNI (You Ain’t Gonna Need It) advises developers to avoid adding features that are not immediately necessary. It aligns with the KISS principle by discouraging unnecessary complexity and focusing only on essential functionality to keep systems simple and secure. </details>

5. How does the Rule of Least Power contribute to simpler and more secure systems?

<details> <summary>Show answer</summary> The Rule of Least Power*recommends using the least powerful language or technology needed for a task. Simpler tools—like static HTML instead of JavaScript—are easier to secure, maintain, and analyze, supporting the KISS principle’s goal of minimizing complexity and risk. </details>
...
> ℹ Trust but Verify is a less strict version of Zero Trust, where access is conditionally granted but still monitored for risks.
...
2. How does the “Assume Breach” mindset strengthen security in a Zero Trust architecture?

<details> <summary>Show answer</summary> “Assume Breach” means designing systems under the assumption that attackers may already be inside the network. This leads to stronger monitoring, isolation, and containment controls, reducing the potential damage if a compromise occurs. </details>
...
4. How does the “Trust but Verify” principle differ from Zero Trust?

<details> <summary>Show answer</summary> “Trust but Verify” allows conditional access after trust is granted but continues to monitor for risks, while Zero Trust eliminates implicit trust entirely. In Zero Trust, every action is continuously verified, even for authenticated users or devices. </details>
...
1. **Proactive, Not Reactive:** Privacy by Design encourages organizations to anticipate and prevent privacy issues before they arise, rather than responding to them after a breach occurs. This means that privacy considerations should be at the forefront of decision-making from the very beginning.
2. **Data Minimization:** One of the core tenets of Privacy by Design is that organizations should only collect, store, and process the minimum amount of personal data necessary to achieve their objectives. This reduces the risk of data exposure and simplifies the overall security landscape.
3. **Default Privacy Settings:** Systems and applications should be configured with the highest level of privacy protection by default, ensuring that users’ personal data is safeguarded without requiring them to manually adjust privacy settings. This aligns with the principle of “secure by default.”
4. **End-to-End Security:** Privacy by Design also means incorporating robust security measures to protect personal data throughout its entire lifecycle, from collection and storage to transmission and disposal. Strong encryption, access controls, and regular security audits are essential to safeguard privacy.
5. **Transparency:** Organizations must be transparent about their data collection and processing practices, clearly informing users of how their personal information will be used, stored, and shared. This transparency helps build trust and ensures compliance with privacy regulations.
6. **User Control:** Giving users control over their own personal data is another critical element. Users should be able to access, correct, or delete their data, and they should have the ability to opt-out of data collection when possible.
...
<details> <summary>Show answer</summary> Default privacy settings ensure that systems are configured with maximum privacy protection by default, without requiring users to change settings manually. This prevents accidental data exposure and aligns with the “secure by default” approach. </details>

4. What does end-to-end security mean in the context of Privacy by Design?

<details> <summary>Show answer</summary> End-to-end security involves protecting personal data throughout its entire lifecycle—from collection and storage to transmission and disposal. It relies on strong encryption, access controls, and regular audits to ensure continuous data protection. </details>
...
> ⚠ Security is a team effort—your cloud, their infrastructure, our responsibility
...
<details> <summary>Show answer</summary> The customer manages everything built on top of the provider’s infrastructure, such as operating systems, data protection, identity and access management, application security, and proper configuration of cloud resources. Customers must also implement strong authentication and data encryption. </details>

4. Why is a clear division of responsibilities critical in cloud security?

<details> <summary>Show answer</summary> A clear division prevents security blind spots where both parties assume the other is responsible. By defining who secures each layer—such as infrastructure vs. applications—organizations can avoid misconfigurations and improve accountability and control. </details>
