## 3.04 Understand Security Capabilities of Information Systems (e.g. memory protection, TPM, …) ##
...
| ---------------------------------- | ----------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| **Boot Integrity**                 | Measures and verifies boot components to detect unauthorized changes before OS loads (measured boot). | Cannot prevent tampering by itself — only detects it.                                        |
| **Key Protection**                 | Stores cryptographic keys securely in hardware, resistant to software-based attacks.                  | Keys are tied to the physical device — recovery is complex if the motherboard or TPM fails.  |
| **Disk Encryption Support**        | Enhances tools like BitLocker by ensuring keys are released only if system integrity is verified.     | Dependent on OS or software integration; TPM alone doesn’t encrypt data.                     |
