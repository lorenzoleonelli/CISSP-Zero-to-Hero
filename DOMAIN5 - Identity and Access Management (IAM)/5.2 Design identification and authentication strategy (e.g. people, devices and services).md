...
> 🔗 According to [Microsoft](https://learn.microsoft.com/en-us/azure/well-architected/security/identity-access), a role is `A set of permissions that define what a user or group can do`. For instance, a "Sales Manager" role might include permissions to access customer data, create sales orders, and manage sales teams.

> ✓ What distinguishes a group from a role ? A role or a user can act as a "principal" - They can be responsible as actors for some actions. A group is a collection of users with shared characteristics or responsibilities that cannot take action. For example, the Sales department cannot take any action, even though we colloquially say so. Action is taken either by a specific person or a person in a role, for example a Sales Manager.
...
1. **Authentication** is the process (happening right after Identification) of verifying the identity of a user, device, or system. It ensures that the entity attempting to gain access is who they claim to be. Think of authentication like different ways to convince the system that you’re the real person.

**Type 1: Something You Know**. This is information inside your head. If someone steals it or sees it, they can use it too.

Examples: Password, PIN, Security questions, Passphrase

> ⚠ Easy to use, easy to steal.

**Type 2: Something You Have**. A physical or digital object that belongs to you. If you don’t have the object, you can’t authenticate.

Examples: Mobile phone receiving an OTP, Smart card, Hardware token (RSA, YubiKey), Digital certificate stored on a device, A mailbox

> ⚠ If someone steals the device, the factor is compromised — unless it’s locked by another factor.

**Type 3: Something You Are**. Biometric traits — physical or behavioral characteristics.

Examples: Fingerprint, Face ID, Iris scan, Voice recognition, Hand geometry

> ⚠ These are convenient but can’t be changed if compromised.

**Type 4: Somewhere You Are**. Authentication based on your location.

Examples: Logging in only from inside the office, GPS-based authentication, Geo-fencing restrictions

> ⚠ Useful for stopping remote or foreign attacks.

**Type 5: Something You Do (Behavioral)**

Patterns or behaviors that are unique to you.

Examples: Typing rhythm, Mouse movement, How you hold your phone

> ⚠ Often used in continuous authentication. And usually considered Type 3 (Something you are) authentication methods.

**Context-Aware Authentication (Adaptive)** This isn’t a new factor — it’s an intelligent combination of signals.

The system evaluates: Time of day, Device health, Network you're using, Past behavior

⚠ If something looks “off,” the system increases verification requirements.

| Authentication Type             | What It Means           | Examples               | Strength / Weakness                 |
| ------------------------------- | ----------------------- | ---------------------- | ----------------------------------- |
| **Type 1** — Something You Know | Info in your memory     | Password, PIN          | Easy but weakest (can be stolen)    |
| **Type 2** — Something You Have | Physical/digital object | Smart card, OTP, token | Stronger, but can be stolen         |
| **Type 3** — Something You Are  | Biometric trait         | Fingerprint, Face ID   | Hard to steal but cannot be changed |
| **Somewhere You Are**           | Location-based          | GPS, office network    | Good for restrictions               |
| **Something You Do**            | Behavioral patterns     | Typing rhythm          | Great as an added layer             |
| **Context-Aware**               | Risk-based decision     | Device, time, behavior | Strongest when combined with MFA    |
...
Examples of practical authentication methods are:
...
- **Multi-Factor Authentication (MFA)**: Extends 2FA by requiring multiple verification methods, such as a password, biometric data (e.g., fingerprint), or a hardware token. A truly secure 2FA or MFA system requires multiple independent authentication factors of different types and that are hard to steal or fake, and includes resistance to phishing and replay attacks—so even if one factor is compromised, attackers still can't access your account.
...
Interested in biometrics ? The table below is for you:
<img width="650" height="412" alt="biometrics" src="https://github.com/user-attachments/assets/b465b0d5-706f-40e4-91ad-b4e078b8b01c" />
...
> ℹ Online Identity Proofing with **Knowledge-Based Authentication (KBA)**. In digital contexts, identity proofing often uses knowledge-based authentication (KBA). For example, when setting up an online account, users might be prompted to answer specific questions that only they should know, such as past addresses, car payments, or the make of a recent purchase. These questions are based on independent sources, like credit bureaus, and users are given limited time to answer.
...
> 🔗 IAL and AAL have been defined in [1.2 Understand and apply security concepts](../DOMAIN1%20-%20Security%20and%20Risk%20Management/1.2%20Understand%20and%20apply%20security%20concepts.md)

> 🔗 Revision 4 of [SP 800-63](https://pages.nist.gov/800-63-4/) , Digital Identity Guidelines, intends to respond to the changing digital landscape that has emerged
...
1. **Broker and Remove Access.** In this method, users need to provide a reason for why they need access to a specific system or resource for a set amount of time. Users typically have a shared privileged account, but the credentials for that account are securely managed and rotated in a central system (vault). Access is granted for the required period and then removed afterward.
2. **Ephemeral Accounts.** These are temporary, one-time-use accounts that are created when needed and automatically deleted or deactivated after the task is completed. This ensures that the account can only be used once, reducing the risk of long-term misuse.
3. **Temporary Elevation.** This method allows users to temporarily gain higher-level permissions or privileges to perform specific tasks. Access is granted on request and for a limited time, after which the elevated privileges are automatically revoked once the time is up, ensuring the user doesn’t retain unnecessary access.
...
sequenceDiagram
    participant User
    participant PolicyAdmin as "Policy/Admin"
    participant System

    User->>PolicyAdmin: Requests privileged access
    PolicyAdmin-->>User: Approve or Deny request
    alt Approved
        PolicyAdmin->>User: Send access granted notification
        ...
        PolicyAdmin->>User: Send access denied notification
    end
