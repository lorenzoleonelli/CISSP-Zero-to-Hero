...
> ℹ A **socket** is an endpoint for communication between two machines over a network. It serves as an interface between the application and the transport layer. A socket allows an application to send or receive data across a network. Each socket is uniquely identified by a combination of the following:
