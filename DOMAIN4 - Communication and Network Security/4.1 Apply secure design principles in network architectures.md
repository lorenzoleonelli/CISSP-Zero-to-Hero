...
1. **Layering (The OSI Model & TCP/IP Model)**. Networks are structured in layers, like a cake with multiple security checks at each level. The OSI model has seven layers (from physical cables to application data), while the TCP/IP model simplifies it to four layers. Each layer plays a role, ensuring secure data transmission and preventing unauthorized access.

2. **Domain Separation (Network Segmentation)**. Not everyone should have access to everything. Domain separation isolates sensitive resources—for example, an HR system shouldn’t be accessible from the public Wi-Fi network. This approach minimizes damage in case of a breach. Techniques like VLANs (Virtual LANs) and subnetting help enforce domain separation.

> ⚠ As businesses continue to adopt cloud computing, remote work, and IoT devices, network security must evolve to keep up with new threats and attack vectors.
...
> ℹ A MAC (Media Access Control) address is a unique identifier assigned to the network interface of a device for communication on a network. It is used in data link layer protocols (like Ethernet and Wi-Fi) to ensure that data packets are correctly addressed to devices on a local network.
...
The **Transport Layer** is `where end-to-end communication is managed`, ensuring that data is properly sequenced and delivered without corruption or loss. This layer uses TCP (Transmission Control Protocol) and UDP (User Datagram Protocol), TCP is like a phone call that requires a confirmed connection and ensures every word arrives in the correct order, whereas UDP is like a megaphone broadcast that sends data quickly without checking if anyone is listening or if they missed a part. 
Attackers frequently exploit TCP-based attacks, such as SYN floods, session hijacking, and port scanning, to disrupt or intercept communications. Rate limiting, deep packet inspection (DPI), and Transport Layer Security (TLS) encryption are commonly used to protect against these threats.
...
There are 2 methods to disconnect a TCP session:
- **FIN** is the "polite" way to disconnect, where both sides say goodbye and finish sending data before hanging up.
- **RST** is the "abrupt" way to disconnect, where one side instantly slams the door shut due to an error or security issue.

> ℹ A **socket** is an end point for communication between two machines over a network. It serves as an interface between the application and the transport layer. A socket allows an application to send or receive data across a network. Each socket is uniquely identified by a combination of the following:
...
> ℹ While both are used for HTTP web traffic, Port 80 is the standard "privileged" gateway for public websites, whereas Port 8080 serves as a popular "non-privileged" alternative frequently used for development, testing, and proxy services.

| Feature         | Port 80                       | Port 8080                        |
| :-------------- | :---------------------------- | :------------------------------- |
| **Primary Use** | Public/Production Web Traffic | Development, Testing, or Proxies |
| **URL Example** | `http://website.com`          | `http://website.com:8080`        |
| **Permissions** | Requires Admin/Root access    | No special permissions needed    |
| **Standard**    | Official HTTP default         | Common "Alternative" convention  |
...
> ℹ In the OSI Session Layer, communication between devices can happen in three different modes:
...
> ✓ *All People Seem To Need Data Protection* … will help you remember the 7 layers of the OSI model.

> ℹ **Non-IP Legacy Protocols** refer to older networking protocols that were used before the widespread adoption of the Internet Protocol (IP). These protocols were often designed for specific network architectures or industries and are still found in legacy systems. Here are a few key ones:
...
The **TCP/IP model** is a `simplified framework that describes how data moves through a network`. It consists of four layers, each handling specific tasks to ensure reliable communication. Unlike the ISO OSI model, which has seven layers, TCP/IP is more practical and widely used in modern networking.
...
> 🔗 [Iana](https://www.iana.org/numbers) is responsible for global coordination of the Internet Protocol addressing systems, as well as the Autonomous System Numbers used for routing Internet traffic.  
...
*Let's try a simple example. You have the network 192.168.1.0/24 (subnet mask 255.255.255.0), which means you have 256 total IP addresses (0-255). You want to subnet this network into 4 smaller subnets.*
...
> 🔗 You don’t need to calculate subnets manually, but you must understand how subnetting impacts security policies, access control, and network architecture. A detailed explanation of subnetting is available [here](https://en.wikipedia.org/wiki/Subnet)

> 🔗 You can learn more about IP addressing and subnetting here: [4.D.1 IPv4 Addressing and Subnetting](4.D.1%20IPv4%20Addressing%20and%20Subnetting.md)
...
> ℹ IEEE 802.1Q is a networking standard that defines how VLAN (Virtual Local Area Network) tags are added to Ethernet frames. This standard allows for multiple VLANs to be transmitted over a single physical network link, enabling network segmentation without requiring separate physical infrastructure.
...
> ℹ The performance of an iSCSI connection largely depends on the underlying network bandwidth. It typically uses standard Ethernet networks, and the bandwidth is determined by the speed of the Ethernet connection (e.g., 1Gbps, 10Gbps, etc.).
...
> ⚠ The **physical topology** refers to the actual, tangible layout of network devices (such as switches, routers, and computers) and how they are physically connected via cables, fiber optics, or wireless links. The **logical topology** describes how data actually flows between devices in a network, regardless of the physical layout. It defines which devices communicate directly and how network protocols operate over the infrastructure.
...
Key Characteristics of east-west traffic are:
...
> 🔗Zero trust is defined in 3.1.8 in [3.1 Research, implement, and manage engineering processes using secure design principles](../DOMAIN3%20-%20Security%20Architecture%20and%20Engineering/3.1%20Research,%20implement,%20and%20manage%20engineering%20processes%20using%20secure%20design%20principles.md)

> ⚠ To implement micro-segmentation, organizations first define their network segments based on security requirements, such as separating sensitive systems (e.g., databases or finance systems) from less critical ones (e.g., employee workstations). Once these segments are identified, policies are created to govern what traffic is allowed between them, ensuring that only authorized communications take place. These policies are enforced through automation, so that any new device or application added to the network is automatically placed in the appropriate segment with the correct policies applied.

> ⚠ Micro-segmentation offers **significant security benefits** by reducing the attack surface and limiting the impact of a breach. However, **it can introduce complexity** in terms of deployment and management, particularly in large or distributed environments. It also requires careful monitoring and ongoing adjustment of policies to ensure the network remains secure and compliant with internal and external regulations. 
...
> 🔗 IDS/IPS are described in detail in 7.2.1 in [](../DOMAIN7%20-%20Security%20Operations/#%207.2%20Conduct%20logging%20and%20monitoring%20activities.md)
...
| **Frequency Band**     | **Key Characteristics**                                 | **Advantages (Pros)**                                                                                                                       | **Disadvantages / Limitations (Cons)**                                                        | **Non-Overlapping Channels**                                                                |
| ---------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| **2.4 GHz**            | One of the oldest and most widely used frequency bands. | Better coverage.<br><br>Better penetration through walls due to lower frequency.                                                            | High congestion and interference (shared with Bluetooth, microwaves, and cordless phones).    | Only **3 channels** (1, 6, and 11) do not interfere with each other out of 14 total.        |
| **5 GHz**              | Modern high-performance band.                           | Significantly higher speeds.<br><br>Less interference.                                                                                      | Shorter range due to higher frequencies.<br><br>Reduced ability to penetrate solid obstacles. | **Multiple channels** available, significantly reducing network congestion.                 |
| **6 GHz** _(Wi-Fi 6E)_ | Introduced with the Wi-Fi 6E standard.                  | Lowest possible latency.<br><br>Minimum interference.<br><br>Tailored for high-density environments (stadiums, large offices, smart homes). | Requires updated, high-performance applications and compatible devices.                       | Offers the **highest number of channels**, specifically engineered for maximum performance. |
...
> ⚠ Wireless networks are inherently more vulnerable than wired connections, but by staying informed and proactive, you can significantly reduce security risks.
...
> ℹ CDNs are not just limited to delivering static content like images and videos. They also help with delivering dynamic content (such as personalized web pages) by utilizing dynamic caching, where personalized elements are cached, or content is fetched from the origin server when necessary.

> ℹ A CDN also provides **high availability** and **fault tolerance**. Since content is distributed across multiple servers, if one server or data center goes down, the CDN can automatically redirect traffic to the next nearest server. This ensures that the website or service remains available even during high traffic periods or in case of server failure.
...
In cybersecurity and network management, data and communication flows are often described using directional terms that help clarify their purpose and security focus.
- **Northbound:** This means communication or data flow going up towards management systems or users. It focuses on making sure there is good visibility, logging, and APIs to monitor and interact with the system.
- **Southbound:** This is communication going down towards the infrastructure like servers, devices, or hardware. The focus here is on controlling and enforcing security policies to protect the infrastructure.
- **Eastbound:** This is internal communication between components or systems within the same environment (sideways). It emphasizes limiting internal movement by using Zero Trust principles and segmentation to reduce risks.
- **Westbound:** This refers to communication going outside the environment, between external systems or partners. The focus is on managing trust boundaries and safely sharing data across those boundaries.

| Direction   | Description                   | Security Focus                 |
|-------------|-------------------------------|-------------------------------|
| Northbound  | Toward management / users      | Logging, APIs, visibility      |
| Southbound  | Toward infrastructure          | Control, enforcement           |
| Eastbound   | Internal lateral movement      | Zero Trust, segmentation       |
| Westbound   | External lateral communication | Trust boundaries, data sharing |
...
> ⚠ With vast amounts of data flowing through the network, it can be difficult to distinguish between meaningful patterns and noise. Administrators need to fine-tune their monitoring systems to focus on the most important metrics and reduce false alarms.

> ⚠ Ensuring the security and privacy of monitoring data is crucial. Sensitive network traffic data must be protected from unauthorized access or tampering to maintain the integrity of the monitoring process.

### Open Questions ###

1. What is the primary distinction between traditional network monitoring and network observability?
<details>
  <summary>Show answer</summary>
Monitoring tells you _that_ a system is failing (state), whereas **observability** uses telemetry and deep data analysis to explain _why_ it is behaving a certain way (behavior). Observability provides actionable insights to predict failures and optimize the network proactively.

</details>

2. A network administrator notices that video conferencing quality drops whenever a large file backup starts. Which technique should be implemented to fix this?
<details>
  <summary>Show answer</summary>
Traffic Shaping. By defining policies that prioritize time-sensitive traffic (voice/video) and rate-limiting less critical traffic (file downloads), the administrator ensures the Quality of Service (QoS) for business-critical applications.
</details>

3. Which component of network management is most concerned with balancing supply and demand to prevent bottlenecks before they occur?
<details>
  <summary>Show answer</summary>
Capacity Management. It involves forecasting future traffic demands, analyzing trends, and scaling infrastructure (adding bandwidth or devices) to ensure the network can handle peak loads without downtime.
</details>

4. In the context of Fault Handling, what is the advantage of a "self-healing" network?
<details>
  <summary>Show answer</summary>
Self-healing mechanisms use **automation** to detect and correct faults (such as rerouting traffic to a redundant link) without human intervention, which significantly reduces the Mean Time to Repair (MTTR) and minimizes service disruption.
</details>
