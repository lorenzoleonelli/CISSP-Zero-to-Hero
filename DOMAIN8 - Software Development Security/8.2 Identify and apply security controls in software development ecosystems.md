...
flowchart TD
    ...
    subgraph "SECURITY"["Security in the CI/CD Pipeline"]
        C1["Shift security left through SAST, SCA, and container scans"]
        C2["Block builds with known vulnerabilities"]
        C3["Verify dependencies and open-source integrity"]
        C4["Use SBOMs and trusted registries"]
        C5["Build security awareness in development"]

        C1 --> C2 --> C3 --> C4 --> C5
    end

    subgraph "ANALOGY"["Assembly Line Analogy"]
        D1["The software pipeline works like a factory line"]
        D2["Each stage adds value or checks for defects"]
        D3["Weak checks allow vulnerabilities to reach production"]
        D4["Strong controls create a safer supply chain"]

        D1 --> D2 --> D3 --> D4
    end

    CI --> CD --> SECURITY --> ANALOGY
