...
> ⚠ The waterfall model is like pouring concrete. Once it sets, there’s no reshaping it.
...
> ℹ  A common Agile pitfall is developers rushing to finish a sprint without considering threats or compliance needs. To combat this, many organizations introduce a **security champion** into each Agile team—someone with enough knowledge to raise a red flag when security is at risk.
...
SAFe is organized in 4 configuration levels:

1. **Essential SAFe**. This is the `core of the framework` — where the Agile Release Train (ART) operates. It includes the teams, program increments, and roles that deliver continuous value through iterative development. It focuses on aligning teams around a shared mission and cadence.

2. **Large Solution SAFe**. This level comes into play when `solutions are too big for a single Agile Release Train`. It coordinates multiple ARTs and suppliers working together to build complex systems or products. It adds roles and practices for solution management and architecture.

3. **Portfolio SAFe**. At this level, `strategic themes, budgeting, and portfolio managemen`t happen. It connects enterprise strategy to execution by organizing multiple programs and large solutions. The focus is on investment funding, governance, and aligning initiatives with business goals.

4. **Full SAFe**. This is the `complete framework`, combining Essential, Large Solution, and Portfolio levels. It’s designed for the largest enterprises that require coordination from team-level delivery all the way up to strategic portfolio management. It provides a comprehensive approach to scaling Agile across the entire organization.
...
> ℹ Security Models give us a structured way to evaluate how well an organization’s processes are functioning and improving over time. 
...
*For example, an organization at Level 1 may respond to a breach with panic and confusion, while a Level 4 or 5 organization will already have a playbook and monitoring in place, likely preventing the breach before it happens or containing it quickly.* 

> ℹ CMM gives us a language and framework for discussing process maturity, but it's general-purpose. 
...
| **Business Function** | **Security Practices (Examples)**                                    | **Focus**                                                                              | **Maturity Progression (Example)**                                                                                                                                              |
| --------------------- | -------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
> ℹ IDEAL isn’t a scoring system like CMM or SAMM—it’s a method for driving meaningful change. That makes it especially useful in environments where security has been neglected or fragmented. It’s also a favorite of consultants because it provides structure to transformation efforts.
...
> ℹ CMM is your altitude—how high your organization is flying. SAMM is your instrumentation—how well you’re navigating securely through software development. IDEAL is your flight plan—where you’re going and how you intend to get there.
...
>⚠ When changes are made without visibility, you end up with what's called **shadow IT**: undocumented changes, unauthorized tools, and mystery behaviors that can blindside operations and security teams alike. 
...
Change management consists of three main components:
1. **Request Control**: This is the process where a proposed change is formally submitted and documented. Anyone who identifies the need for a change—whether it’s fixing a bug, adding a feature, or updating a system—creates a change request that describes what needs to be done and why.
2. **Change Control**: Once a change request is submitted, it goes through change control, which involves reviewing, evaluating, and approving or rejecting the change. This ensures that every change is properly assessed for its impact, risks, and benefits before being allowed to proceed.
3. **Release Control**: After a change is approved and developed, release control manages the actual deployment or rollout of the change into the live environment. This step ensures the change is delivered safely, with minimal disruption, and includes activities like testing, scheduling, and communication.

```mermaid
graph LR
    %% Safe grayscale styling using standard CSS names instead of hex codes
    classDef processNode fill:lightgray,stroke:dimgray,stroke-width:2px,color:black
    
    %% Nodes
    ReqC["<b>1. Request Control</b><br/><br/>• Formal submission and documentation<br/>• Identifies the need (bug, feature, update)<br/>• Describes what must be done and why"]:::processNode
    
    ChgC["<b>2. Change Control</b><br/><br/>• Review, evaluation, and approval/rejection<br/>• Assesses impact, risks, and benefits<br/>• Gates the change before proceeding"]:::processNode
    
    RelC["<b>3. Release Control</b><br/><br/>• Deployment into the live environment<br/>• Includes testing, scheduling, and communication<br/>• Ensures safe delivery with minimal disruption"]:::processNode

    %% Flow
    ReqC -->|Change Request Submitted| ChgC
    ChgC -->|Change Approved & Developed| RelC
    
    %% Safe link styling
    linkStyle default stroke:dimgray,stroke-width:2px,color:black
```

> ℹ The most important aspect of change management is **control**. Control over what goes into production, who made the decision, what risks were considered, and how it can be undone if something goes wrong. Every change should have a rollback plan.
...
> ⚠  IPTs help remove finger pointing. Instead of blaming developers or operations after an incident, everyone owns the outcome together.
