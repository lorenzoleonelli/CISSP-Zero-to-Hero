...
graph TD
  ...
  subgraph Bx[Nodes]
    ...
    B2 --- B3
    B3 --- B1
    Bnote[No single node controls the whole system.\nEach computes & stores data.\nNodes coordinate over the network.]
    Bnote -.-> B1
  end

  %% MIDDLEWARE
  subgraph Cx[Middleware]
    C1[Service Discovery]
    C2[Data Sharing & Caching]
    C3[Coordination & Synchronization]
    C4[Common APIs / Interfaces]
  end

  %% PROTOCOLS
  subgraph Dx[Communication Protocols]
    D1[Transport: TCP / UDP]
    D2[Application: HTTP / gRPC]
    D3[Security: TLS]
    D4[Reliability: ACK / Retry]
  end
