...
stateDiagram-v2
    ...
    state "Bob" as Bob
    state "File.txt" as File

    Alice --> Bob : Grant 'r'
    Bob --> File : Take 'r'
    Bob --> File : Read (✅ if 'r' is held)

    note right of Alice
      Alice has 'r' on File and grants it to Bob
    end note

    note right of Bob
      Bob uses 'Take' to get 'r' from Alice or another source
    end note

    note right of File
      Objects like files can be accessed if rights exist
    end note
```
