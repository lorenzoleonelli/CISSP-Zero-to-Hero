...
stateDiagram-v2
    [*] --> SystemIdle
    SystemIdle --> HighAction: High-level subject performs action
    HighAction --> SystemStateH: System state changes (invisible to low-level users)
    SystemStateH --> SystemIdle

    SystemIdle --> LowAction: Low-level subject performs action
    LowAction --> SystemStateL: System state changes (observable by low-level users)
    SystemStateL --> SystemIdle

    note right of HighAction
      No effect on SystemStateL
