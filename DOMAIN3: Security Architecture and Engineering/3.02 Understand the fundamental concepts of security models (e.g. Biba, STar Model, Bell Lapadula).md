...
stateDiagram-v2
    [*] --> ComponentA
    ComponentA --> ComponentB : Cascading Composition
    ComponentB --> FinalState

    ComponentA --> FeedbackLoop : Feedback Composition
    FeedbackLoop --> ComponentA

    ComponentA --> SharedIO : Hook-Up Composition
    ComponentB --> SharedIO
    SharedIO --> FinalState

    note right of FinalState
      The system is only secure\n
      if all components AND their\n
      interactions maintain security.
    end note
