...
![](../Images/Gemini_Generated_Image_nwij1ynwij1ynwij.png)
...
![](../Images/Pasted%20image%2020260207081852.png)
...
>- Usually Controls are `used in layer`s, combining multiple security controls to develop defense in depth (so that if a layer fails the next one will help in preventing a breach). To better understand this concept, you can imagine a rainbow, to reach the pot of gold you must pass through many different layers.
...
`NIST Cybersecurity Framework (CSF)`: Developed by the National Institute of Standards and Technology (NIST), is a risk-based framework that helps organizations identify, assess, and prioritize their cybersecurity risks and implement appropriate controls to mitigate those risks. It is designed to be flexible and adaptable, and can be customized to fit the specific needs and priorities of an organization.
...
Every family contains many controls (in total there are more than 1000 controls), this allows practitioners to use SP 800–53 also as a comprehensive checklist. In order to apply these controls NIST SP 800–53 prescribes a four-step process:
  3. Select the appropriate security control baselines.
  4. Tailor the baselines.
  5. Document the security control selection process.
  6. Apply the controls.
...
Every of the 5 frameworks above mentioned has its own certifications, SMEs (subject matter experts) and consultants. What was reported above was just a quick excursus to complete the topic of security controls.
...
🔗 Generally, an SCA is a process implemented by federal agencies based on NIST SP 800-53 Rev. 5: https://csrc.nist.gov/pubs/sp/800/53/r5/upd1/final
...
![](../Images/Pasted%20image%2020260207082654.png)
...
<img width="800" height="2000" alt="Risk_concepts" src="https://github.com/user-attachments/assets/3e5c98cd-4123-4023-a6cc-0ef26513d232" />
...
![](../Images/Gemini_Generated_Image_51x6ec51x6ec51x6.png)
### Open Questions ###
...
4. Why do legacy devices pose security risks?
<details>
  <summary>Show answer</summary>
Legacy devices pose security risks due to the lack of vendor support, leaving them vulnerable to newly discovered exploits. Without security updates and patches, these devices become easy targets for attackers.
</details>

5. What are the key distinctions between EOL and EOSL?
<details>
  <summary>Show answer</summary>
EOL (End of Life) marks the cessation of manufacturing, marketing, and sales of a product by the OEM. EOSL (End of Service Life) signifies the termination of all support services, including updates and patches.
</details>

6. What happens to firmware and software updates after a device reaches EOL?
<details>
  <summary>Show answer</summary>
After a device reaches EOL, development of firmware and software updates becomes limited. Post-warranty maintenance may be available at a higher cost for a limited time.
</details>

7. What does the term EOS typically indicate?
<details>
  <summary>Show answer</summary>
EOS (End of Sale) typically indicates the point at which the OEM stops selling the equipment. It's often used interchangeably with EOL.
</details>

8. Do EOL and EOSL dates necessarily mean immediate equipment failure?
<details>
  <summary>Show answer</summary>
EOL and EOSL dates do not necessarily guarantee immediate equipment failure. While the risk of failure increases over time, well-maintained equipment may continue to function effectively.
</details>

9. What options exist for supporting equipment beyond OEM support?
<details>
  <summary>Show answer</summary>
Beyond OEM support, options for supporting equipment include self-maintenance, engaging third-party maintenance providers, or using open-source alternatives for software and firmware.
</details>
...
:link: [The NIST Risk Management Framework](https://csrc.nist.gov/projects/risk-management/about-rmf) provides a process that integrates security, privacy, and cyber supply chain risk management activities into the system development life cycle 
...
**COBIT** is a framework that helps organizations make sure their IT and security activities support business goals instead of operating in isolation. In simple terms, it provides a structured way for management to decide what IT should achieve, how risks should be managed, and how performance should be measured. COBIT focuses on governance, meaning that leadership defines direction and risk tolerance, while IT and security teams implement and operate the necessary controls. The goal is to ensure that technology delivers value, risks are controlled, responsibilities are clear, and the organization can demonstrate that IT investments are aligned with business needs.
The COBIT framework is built around six governance system principles. In plain English, they mean the following:

- Provide stakeholder value – IT and security activities must create value for the business by balancing benefits, risks, and costs. Technology exists to support business objectives, not as an end in itself.

- Holistic approach – Effective governance requires more than processes. People, technology, information, culture, policies, and organizational structures must all work together.

- Dynamic governance system – Governance is not static. The system must adapt when business goals, risks, regulations, or technologies change.

- Governance distinct from management – Leadership sets direction, priorities, and risk tolerance (governance), while management plans, builds, runs, and monitors activities to achieve those objectives.

- Tailored to enterprise needs – There is no one-size-fits-all implementation. COBIT should be adapted based on the organization’s size, industry, risk profile, and strategy.

- End-to-end governance system – Governance applies across the entire organization, covering all information and technology, not just the IT department.

**SABSA (Sherwood Applied Business Security Architecture)** is a security architecture framework that helps organizations design security starting from business needs rather than from technology. In simple terms, SABSA ensures that security controls exist because they support business objectives, protect business value, and manage risk in a structured and traceable way. Instead of beginning with tools or technical solutions, SABSA starts by asking what the business is trying to achieve, what needs protection, and what risks could prevent success, and then builds security architecture layer by layer to support those goals.

The key aspects of SABSA are:

- Business-driven security – Security requirements originate from business goals, risk appetite, and operational needs rather than technical preferences.

- Layered architecture – Security is designed across multiple layers (contextual, conceptual, logical, physical, and operational), moving from business requirements to implementation.

- Traceability – Every security control can be traced back to a specific business requirement or risk.

- Risk-focused approach – Security decisions are based on risk management and business impact.

- Lifecycle integration – Security architecture supports the full lifecycle, including design, implementation, operation, and continuous improvement.

- Alignment with enterprise architecture – Security becomes part of overall enterprise architecture rather than a separate function.
...
| **COBIT**                 | A governance and management framework for enterprise IT that ensures information and technology support business objectives, deliver value, and manage risk effectively. It defines governance principles, processes, and performance measurement to align IT activities with organizational goals. |
| **SABSA**                 | A business-driven security architecture framework that translates business requirements and risk into structured security architecture layers. It ensures that security controls are traceable to business objectives and integrated into enterprise architecture throughout the lifecycle. |
