...
> ⚠ No one is immune to phishing and spear phishing threats. 
...
graph TD
    ...
    %% The Weakest Link Side
    User -->|Weakness| WL[The Weakest Link]
    WL --> S1[Phishing & Social Engineering]
    WL --> S2[Shadow IT & Policy Bypass]
    WL --> S3[Poor Password Hygiene]
    S1 & S2 & S3 --> Threat((Data Breach))

    %% The Crucial Asset Side
    User -->|Strength| AS[The Crucial Asset]
    AS --> R1[Contextual Awareness]
    AS --> R2[First Responder Reporting]
    AS --> R3[Security Culture Champion]
    R1 & R2 & R3 --> Defense((Resilience))

    style WL fill:#ff9999,stroke:#333,stroke-width:2px
    style AS fill:#99ff99,stroke:#333,stroke-width:2px
    style Threat fill:#cc0000,color:#fff
    style Defense fill:#006600,color:#fff
